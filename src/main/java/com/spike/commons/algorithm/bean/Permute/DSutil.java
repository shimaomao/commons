...
class DSutil<E> {
  ...
  static <E> void permute(E[] A) {
    for (int i = A.length; i > 0; i--)
      // for each i
      swap(A, i - 1, DSutil.random(i)); // swap A[i-1] with a random
    // element
  }
  ...
  /**
   * Create a random number function from the standard Java Random class. Turn it into a uniformly
   * distributed value within the range 0 to n-1 by taking the value mod n.
  ...
  public static <E> void render(E[] A) {
    for (E a : A) {
      System.out.print(a + " ");
    }
    System.out.println();
