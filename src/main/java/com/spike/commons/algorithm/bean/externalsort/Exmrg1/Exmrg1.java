package com.spike.commons.algorithm.bean.externalsort.Exmrg1;
...
public class Exmrg1 {
  ...
  public static int reads = 0; // A count of the number of times that we do a
  // read
  public static int writes = 0; // A count of the number of times that we do a

  // write

  // Divide the records in the input file between the two
  // output files, leaving numrecs records in each.
  static void firstpass(DataInputStream in, DataOutputStream out1, DataOutputStream out2,
  ...
  static void dopass(DataInputStream in1, DataInputStream in2, DataOutputStream out1,
      DataOutputStream out2, int numruns, int runlen) throws IOException {
  ...
  static void exmergesort(String infile, String outfile, int recs) throws IOException {
    int temp;
    ...
    DataOutputStream out1 =
        new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FName[IN1])));
    DataOutputStream out2 =
        new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FName[IN2])));
    ...
    for (int i = recs / 2; i > 1; i /= 2) {
      DataInputStream in1 =
          new DataInputStream(new BufferedInputStream(new FileInputStream(FName[IN1])));
      DataInputStream in2 =
          new DataInputStream(new BufferedInputStream(new FileInputStream(FName[IN2])));
      out1 = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FName[OUT1])));
      out2 = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FName[OUT2])));
    ...
    DataInputStream in2 =
        new DataInputStream(new BufferedInputStream(new FileInputStream(FName[IN2])));
    out1 = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(outfile)));
  ...
  public static void main(String args[]) throws IOException {
    ...
    if (((filelength / (4 * NumRec)) * (4 * NumRec)) != filelength) assert false : "The file size must be a multiple of "
        + 4 * NumRec;
