package com.spike.commons.example.nio.channel;
...
import com.spike.commons.example.nio.ChannelUtil;
import com.spike.commons.example.nio.channel.pipe.PipeInJVM;
import com.spike.commons.example.nio.channel.socket.DatagramChannelClient;
import com.spike.commons.example.nio.channel.socket.DatagramChannelServer;
...
public class ChannelTest {
  ...
  private ByteBuffer[] generateByteBuffers(int number) throws Exception {
    ...
    for (int i = 0; i < number; i++) {
      list.add(generateByteBuffer(col1, StringUtils.TAB));
      list.add(generateByteBuffer(col2, StringUtils.TAB));
      list.add(generateByteBuffer(col3, StringUtils.NEWLINE));
    }
