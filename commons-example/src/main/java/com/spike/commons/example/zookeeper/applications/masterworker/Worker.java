...
public class Worker {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    Worker worker = new Worker(ZKConstants.DEFAULT_QUORUM_CONN_STRING);
  ...
  private void createAssignZnode() {
    zk.create(//
      ZKConstants.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId,//
      new byte[0],//
  ...
  private void register() {
    ...
    zk.create(//
      ZKConstants.WORKER_PARENT_ZNODE_PATH_PREFIX + workerName,//
      "Idle".getBytes(),//
      ZKConstants.DEFAULT_ACLS,//
  ...
  private void getTasks() {
    zk.getChildren(//
      ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + workerName,//
      newTaskWatcher, //
      getTaskCallback,//
      null//
    );
  ...
  private Watcher newTaskWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      ...
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if ((ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + workerName).equals(event.getPath())) {
          getTasks();
        }
      }
    }
          ...
          executor.execute(new Runnable() {
            ...
            public void run() {
              ...
              for (String task : children) {
                zk.getData(//
                  ZKConstants.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId + "/" + task,//
                  false, //
                  cb, //
                  task//
                );
      ...
      case OK:
        ...
        executor.execute(new Runnable() {
          ...
          public void run() {
            ...
            zk.create(//
              ZKConstants.STATUS_PARENT_ZNODE_PATH_PREFIX + (String) ctx, //
              "done".getBytes(), //
              ZKConstants.DEFAULT_ACLS,//
              CreateMode.PERSISTENT, //
            ...
            zk.delete(//
              ZKConstants.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId + "/" + (String) ctx,//
              -1,//
              taskVoidCallback, //
              null//
            );
  ...
  private StringCallback taskStatusCreateCallback = new StringCallback() {
    ...
    public void processResult(int rc, String path, Object ctx, String name) {
      ...
      default:
        LOG.error("Failed to create task result data: ", KeeperException.create(Code.get(rc), path));
      }
