package com.spike.commons.example.zookeeper.applications.masterworker;
...
import com.spike.commons.example.zookeeper.applications.masterworker.RecoverSystem.RecoverCallback;
...
public class Master {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    String hostAndPort = ZKConstants.DEFAULT_QUORUM_CONN_STRING;
    ...
    master.createMetadataZnode(ZKConstants.WORKER_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(ZKConstants.ASSIGN_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(ZKConstants.TASK_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(ZKConstants.STATUS_PARENT_ZNODE_PATH, new byte[0]);
  ...
  private Watcher masterExistsWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      if (Event.EventType.NodeDeleted.equals(event.getType())) {
        ...
        if (event.getPath().equals(ZKConstants.MASTER_ZNODE_PATH)) {
  ...
  private void getWorkers() {
    ...
    zk.getChildren(//
      ZKConstants.WORKER_PARENT_ZNODE_PATH,//
  ...
  private Watcher workersChangeWatcher = new Watcher() {
    public void process(WatchedEvent event) {
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if (ZKConstants.WORKER_PARENT_ZNODE_PATH.equals(event.getPath())) {
          getWorkers();
        }
      }
    }
  ...
  private void getAbsentWorkerTasks(String worker) {
    zk.getChildren(//
      ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker,//
      false, //
  ...
  private void recreateTask(RecreateTaskCtx ctx) {
    zk.create(//
      ZKConstants.TASK_PARENT_ZNODE_PATH_PREFIX + ctx.task, //
      ctx.data, //
      ZKConstants.DEFAULT_ACLS, //
      CreateMode.PERSISTENT,//
  ...
  private void getTasks() {
    zk.getChildren(//
      ZKConstants.TASK_PARENT_ZNODE_PATH,//
      tasksChangeWatcher, //
  ...
  private Watcher tasksChangeWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if (ZKConstants.TASK_PARENT_ZNODE_PATH.equals(event.getPath())) {
  ...
  private void getTaskData(String task) {
    zk.getData(//
      ZKConstants.TASK_PARENT_ZNODE_PATH_PREFIX + task, //
      false, //
      taskDataCallback, //
      task//
    );
    ...
    public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
      ...
      case OK:
        ...
        String assignmentPath =
            ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker + "/" + (String) ctx;
  ...
  private void deleteTask(String name) {
    zk.delete(//
      ZKConstants.TASK_PARENT_ZNODE_PATH_PREFIX + name,//
      -1, //
      taskDeleteCallback,//
      null//
    );
