...
public class RecoverSystem {
  ...
  private void getAssignedWorkers() {
    zk.getChildren(//
      ZKConstants.ASSIGN_PARENT_ZNODE_PATH, //
      false, //
    ...
    public void processResult(int rc, String path, Object ctx, List<String> children) {
      ...
      case OK:
        ...
        for (String worker : assignedWorkers) {
          getWorkerAssignments(ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker);
        }
  ...
  private ChildrenCallback getWorkerAssignmentsCallback = new ChildrenCallback() {
    ...
    public void processResult(int rc, String path, Object ctx, List<String> children) {
      ...
      case OK:
        String worker = path.replace(ZKConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX, "");
  ...
  private void recreateTask(RecreateTaskCtx ctx) {
    zk.create(//
      ZKConstants.TASK_ZNODE_PATH_PREFIX + ctx.task,//
      ctx.data,//
  ...
  private void getStatuses() {
    zk.getChildren(//
      ZKConstants.STATUS_PARENT_ZNODE_PATH, //
      false, //
      statusCallback, //
      null//
    );
  ...
  private void processAssignments() {
    ...
    for (String s : assignments) {
      ...
      deleteAssignment(ZKConstants.TASK_PARENT_ZNODE_PATH_PREFIX + s);
      tasks.remove(s);
    }
